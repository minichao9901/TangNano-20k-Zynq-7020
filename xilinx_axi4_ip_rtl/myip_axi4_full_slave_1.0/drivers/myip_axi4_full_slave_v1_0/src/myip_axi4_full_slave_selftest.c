...
/***************************** Include Files *******************************/
#include "myip_axi4_full_slave.h"
...
XStatus MYIP_AXI4_FULL_SLAVE_Mem_SelfTest(void * baseaddr_p)
{
	...
	{
	  MYIP_AXI4_FULL_SLAVE_mWriteMemory(baseaddr+4*Index, (0xDEADBEEF % Index));
	}

	for ( Index = 0; Index < 16; Index++ )
	{
	  Mem32Value = MYIP_AXI4_FULL_SLAVE_mReadMemory(baseaddr+4*Index);
