...
void RTC_Init()
{
	...
	AXI_IIC_SeqWrite_Reg(&AXI_IIC0,0x51,REG8, 0x00, Reset_Data, 2);
}
...
{
	//读取RTC芯片内的时间数据
	AXI_IIC_SeqRead_Reg(&AXI_IIC0,0x51,REG8, 0x02, (uint8_t *)&Time, 7);
...
void Set_Clock(uint8_t hour,uint8_t min,uint8_t sec)
{
	...
	AXI_IIC_SeqWrite_Reg(&AXI_IIC0,0x51,REG8, 0x02, Clock_Data, 3);
}
...
void Set_Date(uint16_t year,uint8_t month,uint8_t day,uint8_t week)
{
	...
	AXI_IIC_SeqWrite_Reg(&AXI_IIC0,0x51,REG8, 0x05, Date_Data, 4);
}
