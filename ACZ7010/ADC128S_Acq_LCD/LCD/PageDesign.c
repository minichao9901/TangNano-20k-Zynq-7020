...
void Draw_Waveform(Wave_Page Wave,uint16_t *Pre_Wave_Data,uint16_t *Wave_Data)
{
	...
	for(i=0;i<len-1;i++)
	{
		...
		LCD_DrawWaveLine(i + Wave.Wave_Area.X1, XAxis_Y - (uint16_t)(Wave.Precision*Pre_Wave_Data[i]),
				i + 1 + Wave.Wave_Area.X1, XAxis_Y - (uint16_t)(Wave.Precision*Pre_Wave_Data[i+1]));
		POINT_COLOR = LCD_YELLOW;
		LCD_DrawWaveLine(i + Wave.Wave_Area.X1, XAxis_Y - (uint16_t)(Wave.Precision*Wave_Data[i]),
				i + 1 + Wave.Wave_Area.X1, XAxis_Y - (uint16_t)(Wave.Precision*Wave_Data[i+1]));
	}
...
void Draw_Normal_Button(Button Button)
{
	...
	//清除按键区域的图形
	Fill_Box(Button.Box,PAGE_COLOR,0);
...

{
	...
	//Draw_Box(Text.Box, LCD_WHITE);
	POINT_COLOR = Text.TextColor;
	BACK_COLOR = Text.BackColor;
	Fill_Box(Text.Box,Text.BackColor,0);//先擦除，后显示
	LCD_ShowString(Text.Box.X1 + X_Offset,Text.Box.Y1 + Y_Offset,150,Text.TextSize+2,Text.TextSize,0,(uint8_t*)str);
