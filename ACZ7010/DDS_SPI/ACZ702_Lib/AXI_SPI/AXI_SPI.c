...
**	@param	Sel_Num			从设备的编号，1~32（如果被设置为从设备，则此处无效）
**	@param	ReadBuffer		接受的数据所存储的数组
**	@param	WriteBuffer		发送的数据所存储的数组
**	@param	BUFFER_SIZE		接收与发送的数据的字节数
**	Sample usage:	AXI_SPI_Transfer(&AXI_SPI0, 1, ReadBuffer, WriteBuffer, 2);//用SPI与1号从机通信，收发2字节
************************************************************************************************************/
...
/************************************************************************************************************
**	@brief	将短数据直接写入SPI发送寄存器
**	@param	SpiInstPtr		SPI对象指针
**	@param	data			待传输数据
**	Sample usage:	AXI_SPI_Sendshort(&AXI_SPI0,0x0FFF);//将0XFFF写入到SPI0的发送寄存器
************************************************************************************************************/
