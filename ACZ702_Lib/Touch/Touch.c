...
void GT911_Init(void)
{
    ...
    IIC_Init(400000);	//初始化I2C,频率为100K
    PS_GPIO_Init();		//初始化PS端GPIO

    PS_GPIO_SetMode(TP_RST_PIN, OUTPUT, 1);		//INT引脚设置为输出1
    PS_GPIO_SetMode(TP_INT_PIN, OUTPUT, 0);		//RST输出为0，复位
    usleep(10000);		//延时10ms
    PS_GPIO_SetPort(TP_INT_PIN, 1);		//RST输出为1，释放复位
    usleep(10000);		//延时10ms
    PS_GPIO_SetPort(TP_RST_PIN, 0);		//INT输出为0
    usleep(100000);		//延时100ms

    GT911_RD_Reg(GT_PID_REG,id,4);		//读取ID
    printf("ID:%s\n",id);		//打印ID
    buff[0]=0X02;
	GT911_WR_Reg(GT_CTRL_REG,buff,1);	//软复位GT911
	GT911_RD_Reg(GT_CFGS_REG,buff,1);	//读取GT_CFGS_REG寄存器
...
void GT911_Scan(Touch_Data *Touch_LCD)
{
	...
	//最高位为1表示数据有效
	if(State & 0X80) {
		GT911_WR_Reg(GT_GSTID_REG, Data_XY, 1);	//写0清寄存器来开启下一次检测
...
//			Touch_LCD->Touched = 1;		//有按键
		...
		for(i=0;i<5;i++) {//for(i=0;i<Touch_LCD->Touch_Num;i++) {
			GT911_RD_Reg(GT911_TPX_TBL[i],Data_XY,4);	//读取XY坐标值
