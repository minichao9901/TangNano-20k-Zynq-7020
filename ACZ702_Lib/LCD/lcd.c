...
void LCD_Init()
{
	...
	memset(frame,0xFF,800*480*BYTES_PIXEL);
	LCD_Refresh();
}
...
void LCD_DisplayPic(uint16_t x, uint16_t y,uint8_t const *Pic)
{
	...
	for(y_pos=y;y_pos<y+height;y_pos++)
	{
		...
		{
			frame[BYTES_PIXEL*x_pos + 800*BYTES_PIXEL*y_pos] =  Pic[Cnt + 8 + 1];
			frame[BYTES_PIXEL*x_pos + 800*BYTES_PIXEL*y_pos + 1] = Pic[Cnt + 8];
			Cnt+=BYTES_PIXEL;
		}
	}
